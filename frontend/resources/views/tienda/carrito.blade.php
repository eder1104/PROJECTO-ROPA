...
<div class="max-w-4xl mx-auto py-20 px-10">
    ...
    @if(empty($itemsCarrito))
        <div class="text-center py-20 border border-dashed border-[#C5A059]/30">
            ...
            <a href="{{ route('tienda.inicio') }}" class="inline-block mt-8 text-xs font-bold underline uppercase tracking-widest">Explorar Colección</a>
        </div>
    @else
        ...
        <div class="bg-white p-10 border border-[#C5A059]/30">
            <div class="space-y-4 text-sm uppercase tracking-widest">
                <div class="flex justify-between">
                    <span>Subtotal</span>
                    <span>${{ number_format($precios['subtotal'], 2) }}</span>
                </div>
                <div class="flex justify-between text-[#C5A059]">
                    <span>Impuestos (IVA)</span>
                    <span>${{ number_format($precios['tax'], 2) }}</span>
                </div>
                <div class="flex justify-between">
                    <span>Envío Premium</span>
                    <span>${{ number_format($precios['shipping'], 2) }}</span>
                </div>
                <div class="flex justify-between border-t border-[#C5A059] pt-6 mt-6 text-xl font-bold">
                    <span>Total</span>
                    <span>${{ number_format($precios['total'], 2) }}</span>
                </div>
            </div>
